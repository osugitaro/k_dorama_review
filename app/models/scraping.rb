...
class Scraping
  def self.dorama_urls
    ...
    current_page = agent.get("https://www.konest.com/contents/drama_list.html?drama_syear=0&cp=2&genre=action")
  ...
  def self.get_dorama(link)
    ...
    agent = Mechanize.new
    page = agent.get(link)
    title = page.at('.tit_title h1').inner_text if page.at('.tit_title')
    cast = page.at('.indent').inner_text if page.at('.tit_title')
    ...
    page.image_with(:src => image).fetch.save("app/assets/images/k_dorama/action/#{id}.jpg")

    unless KDorama.exists?(title: title)
      k_dorama = KDorama.new
      k_dorama.title = title
      k_dorama.cast = cast
      k_dorama.genre = genre
      k_dorama.introduction = introduction
      k_dorama.image.attach(io: File.open(Rails.root.join("app/assets/images/k_dorama/action/#{id}.jpg")), filename: "#{id}.jpg", content_type: 'image/jpg')
      k_dorama.save
    end
  end
end
